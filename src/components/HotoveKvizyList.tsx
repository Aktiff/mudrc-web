...
import { AlertTriangle, Layers, MonitorPlay, Pencil, Play, Plus, Search } from "lucide-react";
...
export default function HotoveKvizyList() {
  ...
  const load = useCallback(async (teams = filterTeams) => {
    setLoading(true);
  ...
  return (
    <div className="space-y-8">
      <div className="bg-brand-card rounded-2xl border border-brand-border p-5 space-y-4">
        <div>
          <label className="label">Filtrovať podľa tímov</label>
          <p className="text-brand-muted text-xs mb-2">
            Zadaj nahlásené tímy (jeden na riadok alebo oddelené čiarkou). Ukáže kvízy, kde tieto tímy ešte nehrali.
          </p>
          <textarea
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={applyFilter} className="btn-primary text-sm py-2.5 px-4 inline-flex items-center gap-2">
        ...
        {filterTeams.length > 0 && (
          <p className="text-sm text-brand-muted">
            Vhodných kvízov: <strong className="text-brand-text">{safeCount}</strong> z {quizzes.length}
          </p>
        )}
      </div>
          ...
          >
            <div className="flex items-start gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <Link href={`/admin/hotove-kvizy/${quiz.id}`} className="font-semibold text-brand-text hover:text-brand-orange-readable transition-colors">
                  {quiz.title}
                </Link>
