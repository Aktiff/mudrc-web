...
const TAG_RULES: TagRule[] = [
  ...
  { tag: "slovensko", patterns: [/\bslovens/, /\bbratislav/, /\bkosic/, /\btatry\b/, /\bdunaj\b/] },
  { tag: "usa", patterns: [/\busa\b/, /\bamerik/, /\bnew\s+york/, /\bwashington\b/, /\bhollywood\b/] },
  { tag: "šport", patterns: [/\bsport/, /\bfutbal/, /\bhokej/, /\bolymp/, /\btitul\b/, /\bliga\b/, /\btenis\b/, /\bformul/, /\bf1\b/] },
  { tag: "veda", patterns: [/\bveda\b/, /\bvedec/, /\batom\b/, /\bgen\b/, /\bplaneta\b/, /\bvesmir/, /\bgravit/, /\bperiodick/] },
  { tag: "astronómia", patterns: [/\bplaneta\b/, /\bslnko\b/, /\bmiesiac\b/, /\bvesmir/, /\bgalax/, /\bkometa\b/, /\borbit/] },
...
export function resolveCustomQuestionTags(
  ...
  const manual = userTags?.map((t) => t.trim().toLowerCase()).filter(Boolean) ?? [];
  if (manual.length) {
    return Array.from(new Set([...manual, "vlastné"])).slice(0, 8);
  }
