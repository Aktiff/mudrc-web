...
export default function AdminUdalostitPage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="space-y-3">
        {events.map((e) => (
          <div key={e.slug} className="bg-white rounded-2xl border border-stone-200 px-6 py-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-stone-400 text-sm">{e.city} &mdash; {e.date} o {e.time}</div>
              </div>
            </div>
            <div className="flex items-center gap-6 text-sm text-stone-400">
              <span>{e.leagueTable.length} tímov v lige</span>
              <span>{e.pastResults.length} výsledkov</span>
              <Link href={`/admin/udalosti/${e.slug}`} className="flex items-center gap-1.5 text-brand-orange hover:underline font-medium">
