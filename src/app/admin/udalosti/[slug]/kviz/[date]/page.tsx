...
export default function AdminQuizDetailPage({ params }: { params: { slug: string; date: string } }) {
  ...
  if (loading) return <div className="p-8 text-stone-400">Načítavam...</div>;

  if (!result) return (
    <div className="p-8">
      <Link href={`/admin/udalosti/${params.slug}`} className="text-brand-orange hover:underline text-sm">Späť</Link>
      <p className="mt-4 text-stone-500">Kvíz nebol nájdený alebo nemá detailné dáta.</p>
    </div>
  );
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href={backUrl} className="text-stone-400 hover:text-stone-700 transition-colors">
            <ChevronLeft className="w-5 h-5" />
          </Link>
          <div>
            <p className="text-xs text-stone-400 font-medium uppercase tracking-wider mb-0.5">Kvíz</p>
        ...
        <div className="flex items-center gap-2">
          ...
          {!editing && (
            <button
              onClick={startEdit}
              className="flex items-center gap-1.5 text-sm text-stone-600 hover:text-brand-orange border border-stone-200 hover:border-brand-orange px-3 py-2 rounded-xl transition-colors"
            >
      ...
      {/* Edit form */}
      {editing && (
        <div className="bg-white rounded-2xl border border-brand-orange/30 p-6 mb-5">
                ...
                {editTeams.map((team, i) => (
                  <tr key={i}>
                    ...
                    <td className="py-2 pl-1">
                      <button onClick={() => removeRow(i)} className="text-stone-300 hover:text-red-400 transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </td>
                  </tr>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-stone-100">
              <th className="text-left px-6 py-3 text-xs text-stone-400 uppercase tracking-wider font-medium w-10">#</th>
              <th className="text-left px-2 py-3 text-xs text-stone-400 uppercase tracking-wider font-medium">Tím</th>
              <th className="text-right px-6 py-3 text-xs text-stone-400 uppercase tracking-wider font-medium">Body</th>
              <th className="text-right px-6 py-3 text-xs text-brand-orange uppercase tracking-wider font-semibold">Liga +</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-stone-50">
            {sorted.map((team, i) => (
              <tr key={i} className={`${i === 0 ? "bg-orange-50/40" : "hover:bg-stone-50/60"}`}>
                <td className="px-6 py-3.5">
                  <span className={`text-sm font-medium ${i === 0 ? "text-brand-orange font-bold" : "text-stone-400"}`}>{i + 1}</span>
                </td>
                <td className="px-2 py-3.5">
                  <span className={`font-semibold text-sm ${i === 0 ? "text-brand-orange" : "text-brand-text"}`}>{team.teamName}</span>
                </td>
                <td className="px-6 py-3.5 text-right">
                  <span className={`font-display text-xl ${i === 0 ? "text-brand-orange" : "text-stone-700"}`}>{fmtNum(team.total)}</span>
                </td>
                <td className="px-6 py-3.5 text-right">
